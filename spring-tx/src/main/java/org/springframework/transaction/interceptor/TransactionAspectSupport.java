...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
	...
	protected Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass,
		...
		if (txAttr == null || !(ptm instanceof CallbackPreferringPlatformTransactionManager)) {
			...
			//创建事物，同时把创建事物过程中得到的信息放到transactioninfo中，transactioninfo是保存当前事物状态的对象
			...
			try {
				...
				//调用使处理沿着拦截器链进行，使最后的目标对象的方法得到调用
				retVal = invocation.proceedWithInvocation();
			}
			catch (Throwable ex) {
				// target invocation exception
				//如果在事物处理方法调用中出现了异常，事物处理如何进行需要根据具体的情况考虑回滚或者提交
				completeTransactionAfterThrowing(txInfo, ex);
				throw ex;
			}
			finally {
				//这里吧与线程绑定的transactioninfo设置为oldtransactioninfo
	...
	protected TransactionInfo createTransactionIfNecessary(@Nullable PlatformTransactionManager tm,
		...
		if (txAttr != null) {
			if (tm != null) {
				//这里使用了定义好的事务方法的配置信息
				//事务穿点由事务处理器来完成，同时返回TransactionStatus来记录当前的事务状态
				//包括已经创建的事务
	...
	protected TransactionInfo prepareTransactionInfo(@Nullable PlatformTransactionManager tm,
		...
		if (txAttr != null) {
			...
			// 它持有管理事务需要的数据，比如，transaction对象就是由TransactionStatus来持有的
		...
		//把当前的transactioninfo与线程绑定，同时在TransactionInfo中由一个变量来保存以前的TransactionInfo，
		// 这样就持有了一连串与事务处理相关的TransactionInfo，虽然不一定需要创建信息的事务，但是总会在请求事务时创建TransactionInfo
		txInfo.bindToThread();
		return txInfo;
	}
